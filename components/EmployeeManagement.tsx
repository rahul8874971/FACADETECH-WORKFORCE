  ...
  return (
    <div className="space-y-8">
      <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200">
        <h3 className="text-lg md:text-xl font-bold text-slate-800 mb-6">Register New Employee</h3>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Full Name</label>
              <input type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full border-slate-300 rounded-xl py-2 px-4 text-sm" required />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Designation / Role</label>
              <input type="text" value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })} className="w-full border-slate-300 rounded-xl py-2 px-4 text-sm" required />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Monthly Salary (₹)</label>
              <input type="number" value={formData.monthlySalary} onChange={e => setFormData({ ...formData, monthlySalary: Number(e.target.value) })} className="w-full border-slate-300 rounded-xl py-2 px-4 text-sm" required />
            </div>
          </div>

          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Join Date</label>
              <input type="date" value={formData.joinDate} onChange={e => setFormData({ ...formData, joinDate: e.target.value })} className="w-full border-slate-300 rounded-xl py-2 px-4 text-sm" required />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Initial Advance (if any)</label>
              <input type="number" value={formData.initialAdvance} onChange={e => setFormData({ ...formData, initialAdvance: Number(e.target.value) })} className="w-full border-slate-300 rounded-xl py-2 px-4 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1.5">Profile Photo</label>
              <input type="file" accept="image/*" onChange={handleFileChange} className="w-full text-[10px] text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[10px] file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
            </div>
          </div>
            ...
            <div className="space-y-4 p-5 bg-slate-50 rounded-2xl border border-slate-200">
              ...
              <div className="mt-4 pt-4 border-t border-slate-200 space-y-3">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Login Credentials</h4>
                <div>
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Initial Password</label>
          ...
          {employees.map(emp => (
            <div key={emp.id} className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => onDelete(emp.id)} className="p-2 bg-rose-50 text-rose-500 rounded-xl">🗑️</button>
              </div>
              <div className="flex justify-between items-center text-xs">
                <div className="flex space-x-4">
                  <div>
                    <span className="text-slate-400 font-bold uppercase text-[8px] block">Salary</span>
                    <span className="font-black text-slate-700">₹{emp.monthlySalary.toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 font-bold uppercase text-[8px] block">Joined</span>
                    <span className="font-black text-slate-700">{emp.joinDate}</span>
                  </div>
                </div>
                {emp.isManager ? (
                  <span className="px-2 py-1 bg-indigo-100 text-indigo-700 rounded-md text-[9px] font-black uppercase tracking-wider">Manager</span>
